...
router.get("/allproduct", async (req, res) => {
  ...
  try {
    // 🔎 Ambil semua produk hanya milik seller ini
    ...
    return res.status(200).json({
      message: `✅ ${products.length} produk dari seller ${sellerInfo.store_name}`,
...
router.get("/by-category/:category_id", async (req, res) => {
  ...
  try {
    ...
    // 🔎 Ambil produk sesuai kategori + filter seller
    ...
    return res.status(200).json({
      message: `✅ Ditemukan ${products.length} produk dalam kategori "${category.name}" milik seller ${sellerInfo.store_name}`,
...
router.get("/:id", async (req, res) => {
  ...
  // 🔎 Cek cache
  const cached = cache.get(`product_${id}_seller_${sellerInfo.id}`);
  if (cached) {
    return res.status(200).json({
  ...
  try {
    const { data: product, error } = await supabase
      ...
      .eq("id", id)
      .eq("seller_id", sellerInfo.id) // 🔒 hanya produk milik seller login
      .single();

    if (error || !product) {
      return res
        .status(404)
        .json({ message: "❌ Produk tidak ditemukan untuk seller ini" });
    }
    ...
    // Cache disimpan per seller
    cache.set(`product_${id}_seller_${sellerInfo.id}`, result);
...
router.put("/:id", uploadForEdit, async (req, res) => {
  ...
  try {
    const { data: oldProduct, error: fetchErr } = await supabase
      ...
      .select("*")
      .eq("id", productId)
      .eq("seller_id", sellerInfo.id) // 🔥 filter by seller_id
      .single();

    if (fetchErr || !oldProduct) {
      return res.status(404).json({ message: "❌ Produk tidak ditemukan / bukan milikmu" });
    }
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    if (type === "variant") {
      ...
      if (varErr || !variant) {
        return res.status(404).json({ message: "❌ Varian tidak ditemukan / bukan milikmu" });
      }
